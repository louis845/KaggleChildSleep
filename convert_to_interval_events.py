...
class IntervalEventsSampler:
    ...
    def sample_single(self, index: int):
        ...
        for event in events:
            onset = (event["onset"] - start) // 12
            wakeup = (event["wakeup"] - start) // 12
    ...
    def sample(self, batch_size: int):
        ...
        for k in range(self.sample_low, self.sample_low + increment):
            accel_data, event_segmentation = self.sample_single(self.shuffle_indices[k])
